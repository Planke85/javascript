...
let allStudents = document.getElementsByClassName("nav-link")[0];
let task1 = document.getElementsByClassName("nav-link")[1];
let task2 = document.getElementsByClassName("nav-link")[2];
let task3 = document.getElementsByClassName("nav-link")[3];
let task4 = document.getElementsByClassName("nav-link")[4];
let task5 = document.getElementsByClassName("nav-link")[5];

allStudents.addEventListener("click", getAllStudents);
task1.addEventListener("click", getTask1);
task2.addEventListener("click", getTask2);
task3.addEventListener("click", getTask3);
task4.addEventListener("click", getTask4);
task5.addEventListener("click", getTask5);
...
function getAllStudents(){
    ...
    resultElement.innerHTML += `
        ...
        <div class="row col-md-10 offset-1 mt-1">
    ...
    for(const student of students) {
        ...
        tableRow += `<td>${student.email}</td>`;
        tableRow += `<td>${student.gender}</td>`;
        tableRow += `<td>${student.city}</td>`;
        tableRow += `<td>${student.averageGrade}</td>`;
        tableRow += `<td>${student.age}</td>`;
        tableRow += `</tr>`;
    };
...
function getTask1(){
    let allStudentsGradeHigherThanTree = students.filter(student => student.averageGrade > 3);

    resultElement.innerHTML = "";
    resultElement.innerHTML += `
        <div class="task">
            <h1>All students with an average grade higher than 3</h1>
        </div>
        <div class="row col-md-10 offset-1 mt-1">
            <table class="table table-dark">
                <thead>
    ...
    let tableRow = "";
    for(const student of allStudentsGradeHigherThanTree) {
        tableRow += `<tr>`;
        tableRow += `<td>${student.firstName}</td>`;
        tableRow += `<td>${student.lastName}</td>`;
        tableRow += `<td>${student.email}</td>`;
...
function getTask2(){
    let allFemaleStudentsGradeHigherThanFive = students
    ...
    resultElement.innerHTML += `
        <div class="task2">
            <h1>All female student names with an average grade of 5</h1>
            <p>${allFemaleStudentsGradeHigherThanFive}</p>
        </div>`;
}

function getTask3(){
    let allMaleStudentWhoLivesInSkopje = students
    ...
    .map(student => `${student.firstName} ${student.lastName}`)
        ...
        resultElement.innerHTML += `
            <div class="task2">
                <h1>All male student full names who live in Skopje and are over 18 years old</h1>
                <h3>${allMaleStudentWhoLivesInSkopje}</h3>
            </div>`;
}

function getTask4(){
    let averageGradesOfAllFemaleStudents = students.filter(student => student.gender === "Female" && student.age > 24)
    let mapStudents = averageGradesOfAllFemaleStudents.map(student => student.averageGrade);
    let reduceStudents =  mapStudents.reduce((sum, student) => sum += student)
    let averageGrades = reduceStudents / averageGradesOfAllFemaleStudents.length

    resultElement.innerHTML = "";
    resultElement.innerHTML += `
        <div class="task2">
            <h1>The average grades of all female students over the age of 24</h1>
            <h2> Average grades: ${averageGrades.toFixed(2)}</h2>
        </div>`;
}

function getTask5(){
    let allMaleStudentNameStartingWithB = students.filter(student => student.firstName[0].toLowerCase() === "b" && student.averageGrade > 2);

    resultElement.innerHTML = "";
    resultElement.innerHTML += `
        <div class="task">
            <h1>All male students with a name starting with B and average grade over 2</h1>
        </div>
        <div class="row col-md-10 offset-1 mt-1">
            <table class="table table-dark">
                <thead>
    ...
    let tableRow = "";
    for(const student of allMaleStudentNameStartingWithB) {
        tableRow += `<tr>`;
        tableRow += `<td>${student.firstName}</td>`;
        tableRow += `<td>${student.lastName}</td>`;
        tableRow += `<td>${student.email}</td>`;
